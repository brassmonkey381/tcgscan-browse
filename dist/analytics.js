...
export function SetAnalytics({ catalog, setId, onOpenCard, }) {
    ...
    if (!priced.length)
        return _jsx(Text, { style: styles.empty, children: "No price data for this set yet." });
...
function SetValueOverTime({ priced }) {
    ...
    return _jsx(ValueOverTimeChart, { title: "Set value over time", series: series, loadingLabel: `aggregating ${priced.length} cards…` });
}
const H = 180;
const PAD = { top: 10, right: 12, bottom: 20, left: 48 };
/** Presentational value-over-time line chart. `series` null = still loading. */
export function ValueOverTimeChart({ title, series, loadingLabel, }) {
    ...
    return (_jsxs(View, { style: styles.chartWrap, children: [_jsxs(View, { style: styles.chartHead, children: [_jsx(Text, { style: styles.chartTitle, children: title }), current != null ? _jsx(Text, { style: styles.chartCur, children: money(current) }) : null] }), _jsx(View, { onLayout: onLayout, style: { width: '100%' }, children: series == null ? (_jsxs(View, { style: [styles.chartState, { height: H }], children: [_jsx(ActivityIndicator, {}), loadingLabel ? _jsx(Text, { style: styles.chartHint, children: loadingLabel }) : null] })) : chart ? (_jsxs(Svg, { width: width, height: H, children: [_jsx(Defs, { children: _jsxs(LinearGradient, { id: "votfill", x1: "0", y1: "0", x2: "0", y2: "1", children: [_jsx(Stop, { offset: "0", stopColor: C.accent, stopOpacity: 0.22 }), _jsx(Stop, { offset: "1", stopColor: C.accent, stopOpacity: 0 })] }) }), chart.ticks.map((t, i) => (_jsx(SvgText, { x: PAD.left - 6, y: chart.y(t) + 3, fontSize: 9, fill: C.sub, textAnchor: "end", children: money(t) }, i))), _jsx(Path, { d: chart.area, fill: "url(#votfill)" }), _jsx(Path, { d: chart.d, stroke: C.accent, strokeWidth: 2, fill: "none", strokeLinejoin: "round" })] })) : (_jsx(View, { style: [styles.chartState, { height: H }], children: _jsx(Text, { style: styles.chartHint, children: "Not enough data for this range" }) })) }), _jsx(View, { style: styles.ranges, children: TIME_RANGES.map((r) => {
                    const on = r === range;
