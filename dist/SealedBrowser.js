...
export function SealedBrowser({ theme: themeProp, languages, numColumns, onOpen, onAdd, addLabel = '＋', }) {
    ...
    return (_jsxs(View, { style: styles.wrap, onLayout: onLayout, children: [_jsxs(View, { style: styles.controls, children: [_jsx(Text, { style: styles.sectionLabel, children: "Sealed" }), _jsxs(View, { style: styles.searchRow, children: [_jsx(TextInput, { value: query, onChangeText: setQuery, placeholder: "Search sealed products\u2026", placeholderTextColor: theme.faint, autoCorrect: false, autoCapitalize: "none", clearButtonMode: "while-editing", style: [styles.search, styles.searchFlex] }), languages ? null : _jsx(LanguageToggle, { theme: themeProp })] }), _jsxs(View, { style: styles.facetGroup, children: [_jsx(Text, { style: styles.facetLabel, children: "Type" }), _jsx(ScrollView, { horizontal: true, showsHorizontalScrollIndicator: false, keyboardShouldPersistTaps: "handled", contentContainerStyle: styles.chipRow, children: [{ key: 'all', label: 'Everything' }, ...SEALED_GROUPS].map((g) => {
                                    const on = group === g.key;
                                    return (_jsx(Pressable, { onPress: () => setGroup(g.key), accessibilityRole: "button", accessibilityState: { selected: on }, style: [styles.chip, on && styles.chipOn], children: _jsx(Text, { style: [styles.chipText, on && styles.chipTextOn], numberOfLines: 1, children: g.label }) }, g.key));
                                }) }), numColumns ? null : (_jsx(View, { style: styles.sizeChips, children: CARD_SIZES.map((s) => {
                                    const on = s === size;
