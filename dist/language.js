...
export const LANGUAGE_ORDER = ['en', 'ja'];
...
export function getBrowseLanguages() {
    return current;
}
/**
 ...
 */
export function setBrowseLanguages(next, opts = {}) {
    const norm = normalizeLanguages(next);
    if (same(norm, current))
...
export function subscribeBrowseLanguages(callback) {
 ...
 */
export async function hydrateBrowseLanguages() {
    if (hydrated)
        return;
    hydrated = true;
    try {
        ...
        if (stored)
            setBrowseLanguages(stored, { persist: false });
    }
    catch {
 ...
 */
export function useBrowseLanguages() {
    ...
    const set = useCallback((v) => setBrowseLanguages(v), []);
    return [langs, set];
}
/**
 ...
 */
export function effectiveLanguages(explicit) {
