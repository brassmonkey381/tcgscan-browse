...
export function labToSrgb(L: number, a: number, b: number): { r: number; g: number; b: number } {
  const fy = (L + 16) / 116;
  const fx = fy + a / 500;
  const fz = fy - b / 200;
...
function dE(x: Lab, y: Lab): number {
  const dL = x.L - y.L, da = x.a - y.a, db = x.b - y.b;
  return Math.sqrt(dL * dL + da * da + db * db);
}

// ---- Path A: on-device index -------------------------------------------------------------

interface ColorMeta {
  regions: string[];
  colors_per_card: number;
  bytes_per_color: number;
...
export class ColorIndex {
  ...
  async load(base: string): Promise<void> {
    const [meta, ids, bin] = await Promise.all([
      fetch(`${base}/card_colors_meta.json`).then((r) => r.json() as Promise<ColorMeta>),
      fetch(`${base}/card_colors_ids.json`).then((r) => r.json() as Promise<string[]>),
  ...
  private static setDist(A: Lab[], B: Lab[]): number {
    let q2c = 0, c2q = 0;
    for (const a of A) q2c += a.w * Math.min(...B.map((b) => dE(a, b)));
    for (const b of B) c2q += b.w * Math.min(...A.map((a) => dE(a, b)));
    return 0.5 * (q2c + c2q);
  }
 ...
 */
export function loadColorIndex(colorUrl?: string): Promise<ColorIndex | null> {
 ...
 */
export function getColorIndex(colorUrl?: string): ColorIndex | null {
