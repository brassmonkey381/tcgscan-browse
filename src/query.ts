...
function parseOwned(v: string): boolean | null {
  if (OWNED_TRUE.has(v)) return true;
  if (OWNED_FALSE.has(v)) return false;
  return null;
}
...
export function scoreCard(
  ...
  opts?: { nameWords?: Set<string>; ownedIds?: ReadonlySet<string> },
): number {
  ...
  if (q.owned !== null && opts?.ownedIds) {
    if (opts.ownedIds.has(card.id) !== q.owned) return 0;
  }
  return score;
}
...
const ART_BAND: Record<string, number> = {
...
export function runQuery<T extends QueryableCard>(
  ...
  if (q.sort === 'relevance') {
    ...
    const band = (c: T): number => (themed ? (ART_BAND[c.fullArtKind ?? ''] ?? 2) : 0);
    // Tiebreak by id so this matches the server's `search_cards` ordering exactly (warm == cold).
    scored.sort(
      (a, b) => band(a.card) - band(b.card) || b.score - a.score || a.card.id.localeCompare(b.card.id),
    );
...
export function describeQuery(q: ParsedQuery, matched: QueryableCard[] = []): string {
  ...
  if (q.owned !== null) parts.push(q.owned ? 'owned' : 'missing');
