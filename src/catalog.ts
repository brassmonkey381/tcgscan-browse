...
export interface CatalogCard {
  ...
  /** One sentence describing what is HAPPENING in the card's artwork, generated by a local
   *  vision model. Present only for the printings the pipeline ships captions for (currently
   *  the Illustration Rares); '' everywhere else, including every JP card. Addressed by the
   *  `theme:` field — never by a bare word, because caption prose is full of words like
   *  "energy" and "powerful" that would wreck ordinary search. */
  sceneCaption?: string;
 ...
 */
export function buildCatalog(raw: RawCatalog, onProgress?: (fraction: number) => void): Promise<Catalog> {
