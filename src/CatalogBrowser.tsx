...
export function CatalogBrowser({
   ...
   */
  const similarLocked = (): boolean => {
    if (!isLocked(lockedFeatures, 'findSimilar')) return false;
            ...
            {searching && serverClamped && serverTotal > serverCards.length ? (
              serverDegraded ? (
                ...
                <Pressable
                  ...
                  accessibilityRole="button"
                  accessibilityLabel={`${serverTotal - serverCards.length} more matches, unlock every match`}
                  onPress={() => onLockedFeature?.('themeSearch')}>
                  <Text style={styles.meterCount}>+{serverTotal - serverCards.length} more matches</Text>
                  {/* THE OFFER, NOT THE PRICE. The dialog this opens names the plan and sells it;
                      naming it here too made the row an argument rather than a door. */}
                  <Text style={styles.meterHint}>Showing the top {serverCards.length}. Unlock every match →</Text>
                </Pressable>
              )
            ) : null}
            {footer}
...
function CollectionChip({
  ...
  styles: Styles;
  onCycle: () => void;
  /** true = owned only, false = missing only, null = everything. */
  state: boolean | null;
}) {
  return (
    <Pressable
      onPress={onCycle}
      style={[styles.facetToggle, state != null && styles.facetToggleOn]}
      accessibilityLabel={
        state === true ? 'Showing cards you own' : state === false ? 'Showing cards you are missing' : 'Collection filter'
      }>
      <Text style={[styles.facetToggleText, state != null && styles.facetToggleTextOn]}>
        {state === true ? '✓ Owned' : state === false ? '◇ Missing' : 'Collection'}
      </Text>
    </Pressable>
  );
}
  ...
  return (
    <View style={styles.facetBar}>
      <View style={styles.facetHeader}>
        ...
        <View style={styles.taxBarSpacer} />
        <SizeChips styles={styles} size={size} onPickSize={onPickSize} />
      </View>
    </View>
  );
